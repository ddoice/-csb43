...
def test_aeb43sample1_short_name(aeb43sample1: Batch):
    "short name"
    accounts: list[Account] = aeb43sample1.accounts or []
    ref = tuple("José Müller Muñiz Avinyò" for _ in accounts)
    result = tuple(acc.short_name for acc in accounts)
    assert result == ref


def test_aeb43sample1_bank_code(aeb43sample1: Batch):
    "short name"
    accounts: list[Account] = aeb43sample1.accounts or []
    ref = tuple("0001" for _ in accounts)
    result = tuple(acc.bank_code for acc in accounts)
    assert result == ref


def test_aeb43sample1_branch_code(aeb43sample1: Batch):
    "short name"
    accounts: list[Account] = aeb43sample1.accounts or []
    ref = tuple("0005" for _ in accounts)
    result = tuple(acc.branch_code for acc in accounts)
    assert result == ref
...
def test_non_convertible_initial_balance(record: Account, amount):
...
@pytest.mark.parametrize("amount,ref", [
    ...
    # truncation
    ("123456.876", Decimal("123456.88")),
    (123456.876, Decimal("123456.88")),
], ids=repr)
def test_invalid_initial_balance(record: Account, amount, ref):
    "setting an invalid branch must raise an exception"
    if not record.get_context().strict:
        with pytest.warns(ValidationWarning):
            record.initial_balance = amount
        assert record.initial_balance == ref
    else:
        with pytest.raises(ValidationException):
            record.branch_code = amount
